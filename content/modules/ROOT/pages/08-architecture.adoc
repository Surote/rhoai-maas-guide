...
| *Gateway*
| Gateway API, Istio (Service Mesh 3)
...
The request hits the `maas-default-gateway` in the `openshift-ingress` namespace. The Gateway API controller (backed by Istio/Service Mesh 3) matches the request against HTTPRoutes to determine which backend to forward to.
